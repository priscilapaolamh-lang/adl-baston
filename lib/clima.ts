...
export async function obtenerClima(ciudad: string) {
  // Si no hay clave API, lanzar error
  if (!API_KEY) {
  ...
  try {
    ...
    return {
      ...
      descripcion: datos.weather[0].description, // Descripción del clima (ej: "cielo claro")
      humedad: datos.main.humidity, // Humedad en %
      ciudad: datos.name, // Nombre de la ciudad
      pais: datos.sys.country, // Código del país
      icono: datos.weather[0].icon, // Código del ícono (ej: "01d", "02n")
    };
  } catch (error) {
